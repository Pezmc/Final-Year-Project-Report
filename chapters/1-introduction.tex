...
Increasing use of debit cards \parencite{bbc2010debit} means that bank statements contain more and more information about where people spend their money. With access to those bank statements now provided online, \lorna{too many withs}with most UK banks offering the option to export \gls{transaction} history, individual users can collate a database of their personal spending habits.
...
%
There are several key parts to this, upload\lorna{uploading?} and parsing of \glspl{transaction} from statements downloaded from a bank\lorna{the users bank?}, resolving the \glspl{reference} found on the statement to the real world business they represent and categorising the individual \glspl{transaction} to make them easier to understand. \question{Is it clear what categorising is?} \gavin{yes}
    
\subsection{Prediction}
Accurately predicting the future \glspl{transaction} that a user will be based on their \glspl{transaction} history.\lorna{accurately predicting future transactions that a user will make based on their transaction history}
%
...
\subsection{Security}
The project should be secure and uphold the high levels of expectation from the users uploading their personal information.\lorna{i dont think this sentence makes sense}
%
The application will deal with information of a sensitive nature, \lorna{therefore}strong security techniques are of high importance to ensure no loss of personally identifiable information. 
% 
